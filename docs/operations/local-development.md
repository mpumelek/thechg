...
The initial organization migration creates only the `organization` schema, `Churches` and `OrganizationalUnits`. The separate Identity context has its own `identity` schema and migrations, including a unique-email index. To review or apply them, substitute `TheChgIdentityDbContext` for `OrganizationDbContext` in the commands above, reviewing its SQL separately. Apply the organization migration first, then Identity, to a verified empty development database. These migrations seed no real denomination, member, account or branch. Country creation is currently restricted by the internal application use case to South Africa (`ZA`, `ZAR`, `Africa/Johannesburg`). The schema remains country-neutral for a later approved Mozambique rollout.
