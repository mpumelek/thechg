...
- This is not a production-ready implementation. The organization domain and migrations exist; an inactive-by-default account store, default-deny web authentication boundary and framework-independent scoped authorization evaluator now exist. There is **no login/invitation flow, persisted scoped grants, membership, payment, messaging or background-job behavior** yet. No organization administration endpoint is exposed.
...
Follow the [Sprint 1 baseline](docs/delivery/sprint-1-rebaseline.md), [Sprint 2 plan and progress](docs/delivery/sprint-2-identity-authorization.md), and [draft backlog](docs/delivery/sprint-delivery-plan.md). Complete persisted scoped grants, audited provisioning and a verified login flow before exposing staff organization data. Payments, member portal, messaging and country expansion follow their approved dependencies and controls.
