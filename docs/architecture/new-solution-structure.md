...
The current solution has an organization model and migration, Identity account persistence with branch-captured pending account requests, persisted dated permission grants, a default-deny Web authentication boundary and scoped authorization. A staff sign-in form, a read-only grant-protected South African branch endpoint and a staff-only account-capture endpoint are exposed. Capture creates no usable credentials, membership or grants. There is no invitation, public self-registration, account approval/activation or grant administration route, nor any outbox, payments, notifications or member portal. The `/health/live` endpoint reports only web-process liveness, not database or provider readiness. No worker jobs are registered. Do not deploy this scaffold as a functioning church system.
