...
The current solution has an organization model and migration, Identity account persistence with an internal invitation/password-enrollment service, persisted dated permission grants, a default-deny Web authentication boundary and scoped authorization. A staff sign-in form and a read-only, grant-protected South African branch endpoint are exposed. There is no public registration, invitation activation or grant administration route, nor any outbox, payments, notifications or member portal. The `/health/live` endpoint reports only web-process liveness, not database or provider readiness. No worker jobs are registered. Do not deploy this scaffold as a functioning church system.
