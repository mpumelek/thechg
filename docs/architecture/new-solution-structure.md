...
The current solution has an organization model and migration, plus Identity account persistence, a separate Identity migration, a default-deny Web authentication boundary and a pure scoped-authorization evaluator. There is no login/invitation flow or persisted permission grant; therefore no staff-only data route is exposed. It has no outbox, payments, notifications or member portal. The `/health/live` endpoint reports only web-process liveness, not database or provider readiness. No worker jobs are registered. Do not deploy this scaffold as a functioning church system.
