...
- This is not a production-ready implementation. The organization domain, inactive-by-default account store, scoped grants, staff sign-in, read-only branch API and branch-captured pending account requests exist. There is no invitation or public self-registration flow, account activation/approval, official membership registration, payment, messaging or background-job behavior yet. No organization administration endpoint is exposed.
...
5. All staff and member registrations are captured in person at branches by authorized staff; no invitations or public self-registration. Capture alone never activates an account, grants staff access or creates official membership. Member accounts and records are distinct. A member's self-service data must be resolved from the authenticated account, never from a caller-supplied member ID alone.
...
Follow the [Sprint 1 baseline](docs/delivery/sprint-1-rebaseline.md), [Sprint 2 plan and progress](docs/delivery/sprint-2-identity-authorization.md), and [draft backlog](docs/delivery/sprint-delivery-plan.md). The next identity work is approved branch-registration authority, private in-person credential setup, staff approval/MFA, audit and a real-cookie end-to-end test. Payments, member portal, messaging and country expansion follow their approved dependencies and controls.
